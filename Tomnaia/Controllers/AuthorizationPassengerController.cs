...
namespace Tomnaia.Controllers
{
    ...
    [ApiController]
    public class AuthorizationPassengerController : ControllerBase
    {
        #region fields
        private readonly IAuthServicePassenger _authServicePassenger;
        #endregion

        #region ctor
        public AuthorizationPassengerController(IAuthServicePassenger authServicePassenger)
        {
            _authServicePassenger = authServicePassenger;
        }
        #endregion

        #region registration
        [HttpPost("register")]
        public async Task<ActionResult> RegisterAsync([FromBody] RegisterPassenger passenger)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            }
            var result = await _authServicePassenger.RegisterAsync(passenger);
            if (result.Succeeded)
            {
        ...
        {
            var success = await _authServicePassenger.ConfirmEmailAsync(email, token);
            if (success)
        ...
        public async Task<ActionResult> LoginAsync([FromBody] LoginUser loginUser)
        {
            ...
            var result = await _authServicePassenger.LoginAsync(loginUser);
            if (result.Success)
                return Ok(result);
        ...
        {
            var result = await _authServicePassenger.LogoutAsync();
        ...
        public async Task<IActionResult> ForgetPasswordAsync(string email)
        {
            ...
            var isSent = await _authServicePassenger.ForgetPasswordAsync(email);
            if (isSent)
            {
