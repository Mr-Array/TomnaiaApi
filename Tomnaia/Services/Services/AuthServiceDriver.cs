...
{
    public class AuthServiceDriver : IAuthServiceDriver
    {
        #region fields
        ...
        #region Registration
        public async Task<IdentityResult> RegisterAsync(RegisterDriver registerDriver)
        {
            var userExist = await _userManager.FindByEmailAsync(registerDriver.Email);

            if (userExist != null)
                return IdentityResult.Failed(new IdentityError { Description = "User with this email already exists." });

            var user = _mapper.Map<User>(registerDriver);
            IdentityResult result = await _userManager.CreateAsync(user, registerDriver.Password);
            if (result.Succeeded)
