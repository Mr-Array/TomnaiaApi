...
{
    public class AuthServiceAdmin : IAuthServiceAdmin
    {
        #region fields
        ...
        #region Registration
        public async Task<IdentityResult> RegisterAsync(RegisterAdmin registerAdmin)
        {
            var userExist = await _userManager.FindByEmailAsync(registerAdmin.Email);
            ...
            IdentityResult result = await _userManager.CreateAsync(user, registerAdmin.Password);
            if (result.Succeeded)
