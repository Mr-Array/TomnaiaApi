...
{
    public class AuthServicePassenger : IAuthServicePassenger
    {
        #region fields
        ...
        #region Registration
        public async Task<IdentityResult> RegisterAsync(RegisterPassenger registerPassenger)
        {
            var userExist = await _userManager.FindByEmailAsync(registerPassenger.Email);

            if (userExist != null)
                return IdentityResult.Failed(new IdentityError { Description = "User with this email already exists." });

            var user = _mapper.Map<User>(registerPassenger);
            IdentityResult result = await _userManager.CreateAsync(user, registerPassenger.Password);
            if (result.Succeeded)
                await _userManager.AddToRoleAsync(user, ConstsRoles.Admin);
